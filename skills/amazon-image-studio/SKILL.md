...
- Whenever a normal chat response includes image prompt planning, also send a visible `模板参考图` section using the already-installed style board assets. If the user has not selected a preset, show the available boards with their preset IDs, mark the recommended default, and let the user reply with an ID; if a preset is selected, show that board beside the plan. Resolve the board from the current user's installed Skill directory, not from the author's machine and not from a malformed path such as `C:/Users/Administrator.codex/...`. Prefer loading the board with the host's local image-preview capability and include the returned image content; a plain file card or ordinary file link does not count as a visible preview. If image preview tooling is unavailable, use an absolute local image path in a Markdown image tag as the fallback. Do not expose only a relative asset path. When the user asks for JSON-only output, preserve `styleTemplate.boardAsset` and do not break the requested JSON format.
