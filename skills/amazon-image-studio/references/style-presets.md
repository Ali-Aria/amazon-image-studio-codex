...
- Resolve `boardAsset` to an absolute local image path or attach the actual image block for the preview. The relative asset path belongs in `styleTemplate.boardAsset`; do not use it as the only user-facing delivery.
